...
export function accent_fold(token: lunr.Token) {
  return token.update(function(str: string) {
    return str.replace(charmapRegExp, function(char: string): string {
      return charmap[char];
    });
  });
}

export default function accent_folding(builder?: lunr.Builder) {
  lunr.Pipeline.registerFunction(accent_fold, 'accentFolding');
  if (builder !== undefined) {
    builder.pipeline.add(accent_fold);
    builder.searchPipeline.add(accent_fold);
  }
}
